...
with manager.connect(
  ...
  print("Testing candidate-preview RPC + commit...\n")
  ...
  try:
    ...
    print("\nCommitting candidate changes to running datastore...")
    commit_reply = m.commit()
    print("✓ Commit succeeded!")
    print(f"Response: {commit_reply.ok}")
    committed = True
    ...
    print("\nDiscarding candidate changes...")
    try:
      m.discard_changes()
      print("✓ Candidate changes discarded.")
    except RPCError:
      print("✗ Could not discard candidate changes.")
  finally:
    if not committed:
      # Safety cleanup if we never reached a successful commit.
      try:
        m.discard_changes()
      except RPCError:
        pass
